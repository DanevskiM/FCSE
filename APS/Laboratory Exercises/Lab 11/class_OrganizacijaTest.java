...
public class OrganizacijaTest {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            map.put(parts[1],first);
            map.put(parts[3],second);
            graph.addEdge(first,second,Integer.parseInt(parts[4]));
        }
        String president = scanner.nextLine();
        System.out.println(graph.prim(map.get(president))
        .stream().mapToInt(Edge::getWeight).sum());
    }
}